...
- **M-2** 완료 (2026-07-07) — RegisterWorker가 지오코딩 실패 시 `(0,0)` 대신 null 저장(커밋 `d8f4368`,
  병합 `2754709`). 뷰 `public_profiles`에 `좌표 null/(0,0) 제외` 조건 추가 → 기존 (0,0) 25건이 지도에서 사라짐.
  (EditProfile은 실패 시 기존 좌표 유지라 안전; 기존 25명은 프로필 주소 재저장으로 자가 복구 가능)
- **L-2** 완료 (2026-07-07) — RegisterWorker/RegisterHospital `console.error`에서 사용자 UUID 제거.

**남음 (미룸/결정대기)**
- **L-1 Phase 2** auth 계정(이메일+해시)까지 완전 삭제 — `service_role` Edge Function 필요(클라 불가, 사용자 배포 필요).
  개인정보(프로필)는 Phase 1에서 이미 파기됨. 위험 낮아 글로벌(다국어) 작업 이후로 미룸.
- **L-3** 하드코딩된 고객센터 이메일 (`EditProfile.tsx:472`, `EditHospital.tsx:287`) — *교체할 대표 이메일 결정되면 1줄 반영.*

> **보안 라운드 사실상 완료.** 다음: 글로벌(다국어) 세팅 — 아래 "다국어 설계" 참조.
